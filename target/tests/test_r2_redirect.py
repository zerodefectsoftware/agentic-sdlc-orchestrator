...
from conftest import P95_BUDGET_SECONDS, TRICKY_URL, read_total
...
def test_redirect_location_is_byte_identical_to_stored_url(client, create_link, url):
    ...
    escapes, stripping a fragment. Any of them breaks "byte-identical".
    """
...
def test_redirect_uses_301_permanent(client, create_link):
    ...
    A1 was resolved to 301 by a human, with the consequence recorded in E4:
    caches absorb repeat visits, so click counts under-report. That trade-off
    was accepted deliberately, which makes the status code a contract rather
    ...
    assert response.status_code == 301, (
        f"A1/E4 fixed the redirect at 301, got {response.status_code}"
    )


def test_expired_code_returns_410(client, create_link):
    """AC2.1 — an expired code no longer resolves; it is 410 Gone (E17).

    Expiry is evaluated lazily at redirect time, so the code must stop
    resolving on its own without a sweeper having run.
    """
    expires_at = datetime.now(timezone.utc) + timedelta(seconds=1)
    body = create_link("https://example.com/expires", expires_at=expires_at.isoformat())
    code = body["code"]

    assert client.get(f"/{code}").status_code == 301, "link should be live before expiry"

    time.sleep(1.5)

    response = client.get(f"/{code}")
    assert response.status_code == 410, (
        f"an expired code must be 410 Gone (E17), got {response.status_code}"
    )
...
def test_unknown_code_records_no_analytics_event(client, create_link, wait_for_total):
    ...
    Two observable consequences: the unknown code discloses no analytics of
    its own, and a real link's count is untouched by traffic to codes that do
    not exist.
    """
    body = create_link("https://example.com/untouched")
    code = body["code"]

    client.get(f"/{code}")
    settled = wait_for_total(client, code, 1)
    baseline = read_total(settled)
    ...
    # Give any (incorrectly) enqueued event the full freshness window to appear.
    time.sleep(1.0)
    after = read_total(client.get(f"/api/links/{code}/analytics").json())
...
def test_redirect_p95_latency_under_budget(client, client_pool, create_link):
    ...
    sustained 50 rps. It cannot certify the production number.
    ...
    queue (E10) precisely to protect this budget. An implementation that
    writes the click synchronously, or that resolves the code with a table
    scan instead of the primary-key lookup E15 assumes, blows the budget here
    long before it reaches a load generator.
    """
    code = create_link("https://example.com/latency")["code"]

    for _ in range(20):  # warm up connections, lazy imports and the ORM cache
        client.get(f"/{code}")
