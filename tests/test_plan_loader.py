...
def test_the_implementer_cannot_touch_the_suite_judging_it(plan):
    """D6: a child may write its own module and nothing else, least of all the tests."""
    template = plan.node("impl").template
    assert template.write_scope == ["target/shortener/{item.path}/**"]
    assert template.freeze_paths == ["target/tests/**"]


def test_each_module_is_implemented_by_its_own_agent(plan):
    """D24: the fan-out is safe once the contract is settled, so it comes back.

    The first fan-out failed on names — a module importing exception types from
    a sibling whose author had not run. That is now decided by the architect and
    generated by `scaffold`, so the children agree by construction rather than
    by luck.
    """
    impl = plan.node("impl")
    assert impl.kind is NodeKind.FANOUT
    assert impl.from_ == "design.artifacts.modules"
    assert "design.artifacts.interfaces" in impl.template.inputs


def test_a_module_gate_can_require_its_own_imports_to_resolve(plan):
    """The check that would have caught the original failure, at the right node.

    Before the contract existed this was unsatisfiable per module: a child's
    imports could not resolve until its siblings landed, so every earlier child
    would have failed for work that was never its own. Stubs make it meaningful
    at the child's own gate rather than two nodes later.
    """
    checks = [str(check) for check in plan.node("impl").template.gate.checks]
    assert "imports.resolve == true" in checks


def test_the_architect_decides_the_names_before_anyone_writes_them(plan):
    """D24: the contract is an output of design, and its gates check it is usable."""
    design = plan.node("design")
    assert "interfaces" in design.outputs

    checks = [str(check) for check in design.gate.checks]
    assert "every_module_has_an_interface()" in checks
